...
lesson:
  prerequisite: "SPEC.md・DESIGN.mdとNode.js・Gitを用意する"
  output: "Spec Kitを導入した作業ディレクトリと開発原則"
  completion: "既存ファイルを保持し、導入したCLIの版とプロジェクト構造を確認する"
...
公式リポジトリのリリースタグを指定すると、入手元と版を明確にできます。以下は[Spec Kit 1.0.0](https://github.com/github/spec-kit/releases/tag/v1.0.0)を使う例です。別の版を選ぶ場合は、その版のCLIヘルプも確認してください。
...
```bash
uvx --from git+https://github.com/github/spec-kit.git@v1.0.0 specify init --here
```
:::
