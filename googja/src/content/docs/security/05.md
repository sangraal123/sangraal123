...
lesson:
  prerequisite: "認証とデータ構造、Firestore・Storageルールを用意する"
  output: "所有権・入力・削除を確認する権限チェックリスト"
  completion: "本人・他人・未ログインの操作を検証し、サーバー側の認可も確認する"
  environment: "ソースコード・設定・開発ビルドまたはEmulator"
  effort: "45〜120分"
  cost: "ローカル確認はクラウド不要。実サービス検証は利用条件による"
---
...
1. [セキュリティルールでアクセスを制御する理由](#1-セキュリティルールでアクセスを制御する理由)
...
flowchart LR
    ...
    R["セキュリティルール<br/>クライアントからのアクセスを判定"] --> D["Firestore のデータ"]
```

**アプリ側のチェックはUXのため、セキュリティルールは防御のため。** この2つは役割がまったく違います。

これはモバイル／WebのクライアントSDKからのアクセスについての説明です。**Admin SDKなどのサーバーSDKはFirestoreルールを迂回する**ため、サーバー側の認可とIAMも必要です。App Checkは別の層で不正利用を減らします。[公式のルール構造](https://firebase.google.com/docs/firestore/security/rules-structure)を参照してください。
...
**① 全拒否は、別の許可を取り消さない**
Firestoreは一致するルールの**どれか1つが許可すればアクセスを認めます**。末尾の `if false` は既定の拒否を明示するだけで、先に書いた広すぎる許可を上書きできません。`users/{userId}/{document=**}` で全項目を許可すると、有料プランや管理用の値まで本人が書き換えられる可能性があるため、必要なコレクションだけに絞ります。
...
さらに厳しくするなら、上の `habits` 用ルールを**次のルールに置き換えます**。広い `allow write` と併記すると、検証に失敗しても別の許可が通してしまいます。
...
これで、作成時も更新時も空文字や極端に長いタイトルを拒否できます。必要に応じて `keys().hasOnly(...)` で許可フィールドを限定し、型・値の範囲・変更不可の項目も検証します。
...
削除では `request.resource` が存在しないため、アップロードの検証とは分けます。Content-Typeはクライアントが申告するメタデータなので、画像の実体を保証するものではありません。公開画像を扱う場合は、サーバー側で内容の検証も検討します。
...
さらに、AIのAPIを呼ぶ関数のように**呼ばれるたびに課金が発生する**ものは、認証だけでは不十分です。ユーザーごとの呼び出し回数・入力サイズを制限し、クライアントのApp Check設定と関数の `enforceAppCheck: true` を組み合わせます。**App Checkと回数制限は代替関係ではありません**。正規アプリからの大量呼び出しにも備えます。[App Checkの強制適用](https://firebase.google.com/docs/app-check/cloud-functions)と[応用事項編の実装例](../../advanced/03/)を参照してください。
...
6. 空文字・100文字超のtitleが、作成だけでなく更新でも拒否されること。
7. ユーザー本人でも、管理用フィールドや有料権利を自由に書き込めないこと。
...
- **匿名認証と本人確認・購入権利を区別する**：匿名ユーザーもFirebase上ではUIDを持つ認証済みユーザーで、自分のデータを分離できます。ただし、実在の本人や購入済みであることを証明する仕組みではありません。購入権利はストア検証などで判定します。
...
- [ ] 課金の発生する関数に回数・入力サイズの制限があり、App Checkの強制適用も確認した
