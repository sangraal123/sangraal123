...
この記事のローカル通知は Expo Go でも確認できますが、**リモートプッシュ通知は Expo Go では受け取れません**（SDK 53 以降、Expo Go からリモート通知のサポートが外れています）。§6のリモート通知を試す際は、`eas build --profile development` で作った開発ビルドを使ってください。
:::
