...
lesson:
  prerequisite: "ローカル保存版のMVPとFirebaseプロジェクトを用意する"
  output: "認証とクラウド保存への移行処理"
  completion: "本人以外のアクセスを拒否し、移行失敗からの再試行で重複しない"
  environment: "Firebase JS SDK・Expo Go。ネイティブSDKへ移る場合は開発ビルド"
  effort: "90〜180分"
