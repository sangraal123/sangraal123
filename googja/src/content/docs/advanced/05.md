...
lesson:
  prerequisite: "一度ストアへアップロードし、更新対象と権限を確認する"
  output: "掲載文と素材を検査して送るfastlaneの設定"
  completion: "送信前の差分と送信後の管理画面を照合し、審査提出と区別できる"
  environment: "Ruby・fastlane・ストア管理画面。Expo Goは使用しない"
  effort: "90〜180分"
...
最初に、**APIの機能と、この記事で自動化する範囲**を分けます。
...
App Store Connect APIには[価格・配信地域などを更新する機能](https://developer.apple.com/documentation/appstoreconnectapi/apps)もあります。「このレーンで扱わない」ことと「APIが存在しない」ことは別です。
...
この例では **App Manager** を使いますが、**審査提出なども可能なロール**です。チームキーは特定アプリだけにアクセスを限定できないため、最小権限の要件を満たすか確認してください。用途によってはアプリへのアクセスを絞ったユーザーの個人キーも検討します。以下のIssuer IDを使う手順はチームキー向けです。[AppleのAPIキーの説明](https://developer.apple.com/help/app-store-connect/get-started/app-store-connect-api/)を参照してください。
...
`.p8` ファイルは**一度だけ**ダウンロードできます。ダウンロード済みの秘密鍵を紛失した場合は再取得できないため、そのキーを取り消して新しく発行します。
...
| 漏れた場合 | データを読まれる。流出した情報は回収できない | **許可された範囲で掲載情報やテスト配布を変更されうる** |
...
一方、**「製品版としてのリリース」を渡さなければ、鍵による製品版ビルドの公開を制限できます**。設定を書き間違えても、製品版へのビルド公開権限を抑えられます。ただし、ストア掲載情報の編集権限は残ります。Play Consoleの説明文にも「この権限で、ユーザーはGoogle Playで製品版としてアプリを公開することはできません」と明記されています。
...
- fastlane/metadata/ja/keywords.txt          （UTF-8で100バイト以内、半角コンマ区切り）
...
Apple用キーワードはUTF-8のバイト数も検証してください。
```
...
# 最低限の存在・空欄チェック。翻訳や画像の内容は実行前に目視確認する。
def validate_store_files!(platform, with_screenshots)
  base = File.expand_path(__dir__)
  locales = platform == :ios ? ["ja", "en-US"] : ["ja-JP", "en-US"]
  fields = platform == :ios ? %w[name subtitle keywords description] : %w[title short_description full_description]
  locales.each do |locale|
    root = platform == :ios ? "metadata/#{locale}" : "metadata/android/#{locale}"
    fields.each do |field|
      path = File.join(base, root, "#{field}.txt")
      UI.user_error!("掲載文がありません: #{path}") unless File.file?(path)
      body = File.read(path, encoding: "UTF-8").strip
      UI.user_error!("掲載文が空です: #{path}") if body.empty?
      if field == "keywords" && body.bytesize > 100
        UI.user_error!("キーワードが100バイトを超えています: #{path}")
      end
    end
    next unless with_screenshots
    screenshots = platform == :ios ? "screenshots/#{locale}" : "#{root}/images/phoneScreenshots"
    files = Dir.glob(File.join(base, screenshots, "*.{png,jpg,jpeg}"))
    UI.user_error!("スクリーンショットがありません: #{screenshots}") if files.empty?
  end
end
...
この設定で保留した変更の送信はConsoleで行います。ただし、APIやfastlane全般に審査へ送る能力がないという意味ではありません。`rescue_changes_not_sent_for_review` は既定で有効なため、例では `false` にして、エラー時に設定を変えて再試行する動作を止めています。[supplyのオプション](https://docs.fastlane.tools/actions/supply/)を確認してください。
:::
